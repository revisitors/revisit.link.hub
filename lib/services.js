...
var serviceURLS = serviceList.map(function (s) {
  return s.url;
});
...
exports.add = function (request, reply) {
  ...
  if (!request.payload.services || !request.payload.content) {
    reply.view('error', {
      reason: 'Invalid input: missing at least 1 service and/or image data'
    });
    return;
  }
  ...
  var addService = function (sv) {
    ...
    rvTether.add(service, function (err, svc) {
      if (err) {
        reply.view('error', {
          reason: 'File is too big - we have a 1 MB limit'
        });
        return;
      }
  ...
  imageOps.resize(request.payload.content, function (err, finalContent) {
    if (err) {
      reply.view('error', {
        reason: err.toString().split('Error: ')[1]
      });
      return;
    }
    ...
    urls.forEach(function (sv, idx) {
      if (serviceURLS.indexOf(sv) > -1) {
        setImmediate(function () {
          addService(sv);
        });
      } else {
        urls.splice(sv, 1);
        console.error('invalid url ', sv);
      }
    });
  });
};
